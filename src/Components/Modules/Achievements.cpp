	...
	void Achievements::ChangeItemDefWidth(const char* menuName, const char* itemDefName, int value)
	{
		...
		for (auto i = 0; i < menu->itemCount; i++)
		{
			if (menu->items[i] && menu->items[i]->window.name)
			{
				if (!_stricmp(menu->items[i]->window.name, itemDefName))
				{
					menu->items[i]->window.rectClient.w = value;
				}
			}
		}
	}
	...
	Achievements::Achievements()
	{
		...
		UIScript::Add("achievement_get_info", []([[maybe_unused]] const UIScript::Token& token, [[maybe_unused]] const Game::uiInfo_s* info)
		{
			...
			if (!Game::CL_IsCgameInitialized())
			{
				...
				if (Dvars::Functions::Dvar_FindVar("ui_achievement_page")->current.integer == 0)
				{
					...
					if (Dvars::Functions::Dvar_FindVar("ui_achievement_status")->current.integer == 1)
						ChangeItemDefText("achievements", "date_achievement_received", data_unlocked);
					else
				...
				else if (Dvars::Functions::Dvar_FindVar("ui_achievement_page")->current.integer == 1)
				{
					// Check the achievement status. Update the itemDefs if the achievement has been received
					if (Dvars::Functions::Dvar_FindVar("ui_achievement_status")->current.integer == 1)
						ChangeItemDefText("achievements_page_1", "date_achievement_received", data_unlocked);
					else
				...
				else
				{
					// Check the achievement status. Update the itemDefs if the achievement has been received
					if (Dvars::Functions::Dvar_FindVar("ui_achievement_status")->current.integer == 1)
						ChangeItemDefText("achievements_page_2", "date_achievement_received", data_unlocked);
					else
			...
			else
			{
				...
				if (Dvars::Functions::Dvar_FindVar("ui_achievement_page")->current.integer == 0)
				{
					// Check the achievement status. Update the itemDefs if the achievement has been received
					if (Dvars::Functions::Dvar_FindVar("ui_achievement_status")->current.integer == 1)
						ChangeItemDefText("achievements_ingame", "date_achievement_received", data_unlocked);
					else
				...
				else if (Dvars::Functions::Dvar_FindVar("ui_achievement_page")->current.integer == 1)
				{
					// Check the achievement status. Update the itemDefs if the achievement has been received
					if (Dvars::Functions::Dvar_FindVar("ui_achievement_status")->current.integer == 1)
						ChangeItemDefText("achievements_page_1_ingame", "date_achievement_received", data_unlocked);
					else
				...
				else
				{
					// Check the achievement status. Update the itemDefs if the achievement has been received
					if (Dvars::Functions::Dvar_FindVar("ui_achievement_status")->current.integer == 1)
						ChangeItemDefText("achievements_page_2_ingame", "date_achievement_received", data_unlocked);
					else
		...
		UIScript::Add("achievement_progressbar", []([[maybe_unused]] const UIScript::Token& token, [[maybe_unused]] const Game::uiInfo_s* info)
		{
			Scheduler::Once([]
			{
				...
				int totalWidth = Dvars::console_menu_style->current.enabled ? 253 : 493;
				...
				if (!Game::CL_IsCgameInitialized())
				{
					...
					ChangeItemDefText("achievements", "achievement_total_count_info", achievement_count);
					ChangeItemDefText("achievements_page_1", "achievement_total_count_info", achievement_count);
				...
				else
				{
					...
					ChangeItemDefText("achievements_ingame", "achievement_total_count_info", achievement_count);
					ChangeItemDefText("achievements_page_1_ingame", "achievement_total_count_info", achievement_count);
		...
		GSC::AddFunction("giveAchievement", []
		{
			...
			{
				#ifdef DEMO_VERSION
					// The Demo version has no achievements.
				#else
