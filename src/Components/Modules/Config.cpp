...
namespace Components
{
	std::unordered_map<std::string, Config::field_definition_t> Config::field_definitions =
	{
		...
		{define_field("allow_compact_mod_folder", field_type::boolean, false)},
	};

	std::string Config::GetConfigFile()
	{
		std::string mod_config = "settings.json";
		std::string base_path = Dvars::Functions::Dvar_FindVar("fs_basepath")->current.string + "\\players\\profiles\\"s + mod_config.c_str();
	...
	void Config::CallExecFromCFG(bool useModConfig)
	{
		...
		else
		{
			...
			{
				auto data = Utils::IO::ReadFile(profilePathStock.c_str() + "config.cfg"s);
	...
	{	
		...
		if (!Config::GameWithMod)
		{
			...
			/*if (Utils::IO::FileExists(profilePath.c_str() + "config.cfg"s))
			{
				...
				Utils::IO::RemoveFile(profilePath.c_str() + "dev_config.cfg"s);
				Utils::IO::WriteFile(profilePath.c_str() + "dev_config.cfg"s, data);
			}*/
		}
		else
		{
	...
	Config::Config()
	{
		...
		Scheduler::Once([]
		{
			if(!Utils::IO::FileExists("players/profile/settings.json"))
			{
				Config::Set("language", Config::GetRaw("language"));
				Config::Set("language_first_setting", Config::GetRaw("language_first_setting"));
				Config::Set("allow_compact_mod_folder", Config::GetRaw("allow_compact_mod_folder"));
			}
		}, Scheduler::Pipeline::MAIN);

		//UIScript::Add("update_config_field", []([[maybe_unused]] const UIScript::Token& token, [[maybe_unused]] const Game::uiInfo_s* info)
		//{
		//	char out[1028];
		//	if (Game::String_Parse(token.get<const char**>(), out, 1024))
		//	{
