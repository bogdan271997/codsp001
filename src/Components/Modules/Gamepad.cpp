...
namespace Components
{
	...
	void Gamepad::GetButtonLayout()
	{
		...
		if (gpadButtonConfig == "buttons_default")
		{
			...
			buttonMappingLayout.buttonBack		= "MENU_SP_TIMER";// MENU_NOT_USED
	...
	{
		bool currentState = IsGamePadInUse();
		if(HasGamepadStateChanged(currentState))
		{
			if (currentState)
			{
				Utils::Hook::Set<const char*>(0x40A0B0, "MODERN_OPS_LOC_PLATFORM_RELOAD_GAMEPAD");
			}
			else
			{
				Utils::Hook::Set<const char*>(0x40A0B0, "PLATFORM_RELOAD");
			}

			gamePads->previousState = currentState;
		}
	}

	bool Gamepad::HasGamepadStateChanged(bool currentState)
	{
		bool stateChanged = (currentState != gamePads->previousState);
		gamePads->previousState = currentState;
		return stateChanged;
	}
